...
(function($) {
    ...
    var $window = $(window),
        ...
        _setInlineErrorMessage = function($input, mess, conf, $messageContainer) {
            ...
            if( custom ) {
                ...
                if( $mess.length == 0 && $input.valAttr("showErrorMessage")!="false") {
                    $mess = $('<div></div>').addClass('help-block').addClass(conf.errorMessageClass);
        ...
        _templateMessage = function($form, title, errorMessages, conf) {
            ...
            console.log($form.children().eq(0).before(container));
    ...
    */
    ...
    $.formUtils = {
                        ...
                        if( isValid !== null ) {
                            ...
                            if( !validationErrorMsg ) {
                                ...
                                if( !validationErrorMsg ) {
                                    ...
                                    var element_label = $("label[for='" + $elem[0].id + "']").html();
                                    validationErrorMsg = validationErrorMsg.replace("%s", element_label)
                                }
                            }
                        }
        ...
        * @var {Object}
        */
        LANG : JSON.parse(error_messages)
    };
    ...
    $.formUtils.addValidator({
        ...
        validatorFunction : function(val, $el, config, lang, $form) {
            if($.trim(val)){
                ...
                var element_label = $("label[for='" + $el[0].id + "']").html();
                this.errorMessage = lang.emptyDropdown.replace("%s", element_label);
    ...
    */
    $.formUtils.addValidator({
        ...
        validatorFunction : function(val, $el, conf, lang) {
            ...
            if($el.valAttr("formatted")){
                // remove spaces
                val = val.replace(/\s+/g, '');
            }

            if($el.valAttr("creditcard")){
                // The Luhn Algorithm
                var nCheck = 0, nDigit = 0, bEven = false;
                var _val = val.replace(/\D/g, "");

                for (var n = _val.length - 1; n >= 0; n--) {
                    var cDigit = _val.charAt(n),
                ...
                checkResult = ((nCheck % 10) == 0);
                if(!checkResult) {
                    this.errorMessage = lang.notCreditCardNumber;
                    return checkResult;
                }
            }
            ...
            {   // outside of allowed range
                case "out":
                    if(lengthCheckResults[1]==lengthCheckResults[2]){
                        this.errorMessage = lang.lengthNotExactly.replace("%s", element_label).replace("%d", lengthCheckResults[1]);
                    } else {
                        this.errorMessage = lang.lengthOutOfRange.replace("%s", element_label).replace("%d", lengthAllowed);
                    }
                    checkResult = false;
                    break;
                // too short
                case "min":
                    if(lengthCheckResults[1]==1){   // blank checker

                        this.errorMessage = lang.notBlank.replace("%s", element_label);
                    } else {
                        this.errorMessage = lang.lengthTooShort.replace("%s", element_label).replace("%d", lengthCheckResults[1]);
                    }
                    checkResult = false;
                    break;
                // too long
                case "max":
                    this.errorMessage = lang.lengthTooLong.replace("%s", element_label).replace("%d", lengthCheckResults[1]);
                    checkResult = false;
                    break;
                // ok
                default:
...
$("form").submit(function () {
    $(this).addClass("is-submitted");   // is-submitted makes error messages visible
});
/* end edit */
