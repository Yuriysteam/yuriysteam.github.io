...
function renderChatMessages(msgArray) {
  for (var i = 0; i < msgArray.data.length; i++) {
    ...
    if(n){
      listItem = '<div class="list-item hideLi '+ msgArray.id +'"><div class="buttons-area" data-id="'+ msgArray.id +'">' + msgArray.data[i] + '</div></div>';
    } else {
      listItem = '<div class="list-item hideLi '+ msgArray.id +'"  id="'+ msgArray.id + i +'"><div class="message" data-id="'+ msgArray.id +'">' + msgArray.data[i] + '</div></div>';
    }
    $(".chat-wrapper").append(listItem);

  }

   addAnimation(msgArray.id);
...
function addAnimation(arrayId) {
  var listItem = '.list-item.' + arrayId;
  $(listItem).each(function(i) {
   $(this).delay((++i) * 800).fadeTo(2000, 1).addClass('animation');
 })
}
